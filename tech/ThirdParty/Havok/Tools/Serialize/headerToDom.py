...
class CppParser:
    ...
    re_refVariant = re.compile( r"([\s*<])hkRefVariant([\s*>])" )
    re_junkymacros = re.compile( r"\s*HK_(CPU_PTR|PAD_ON_SPU|THREAD_LOCAL|INIT_FUNCTION)\((.*?)\)" ) ### leave group(2) in place
    ...
    def parseClass(self, match, txt, declaredscope):
        if match.group("decltype") == "{":
            ...
            def appendMember(mname, mtype, mvis, msrc, mflags=""):
                ...
                assert len(mtype)
                if klass.reflected:
                    assert mname.startswith(klass.memberprefix), "Member '%s::%s' does not start with '%s'\n" \
                       "Either add the prefix or add a //+memberprefix declaration to the class" % (
                           klass.name, mname, klass.memberprefix)
                if mname.startswith(klass.memberprefix):
                    mname = mname[len(klass.memberprefix):]
            ...
            while 1:
                ...
                if item == self.re_accessControl:
                    ...
                    if lastParsed == None:
                        ...
                        raise "Unhandled case for attributes, don't know how to apply '%s' to previous item in '%s'" % (attrs, klass.name)
                    if target:
    ...
    def parseString(self, txt):
        ...
        txt = self.re_refVariant.sub(r"\1hkReferencedObject*\2", txt) # hkRefVariant -> hkReferencedObject*
...
def main():
    ...
    for arg in args:
        ...
        import bindingToXml
        print bindingToXml.bindingToXml("doc", document)
